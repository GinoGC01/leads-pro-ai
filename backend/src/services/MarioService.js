...
class MarioService {
    ...
    static async generateStrategy(leadId, previousFailedStrategies = []) {
        try {
            ...
            const agencyContext = settings?.agencyContext || 'Agencia de Marketing y Tecnología';
            const coreServices = settings?.core_services || [];
            const valueProp = settings?.value_proposition || '';

            // 2. RAG Injection: Search mario_knowledge in Qdrant based on Lead's applied tactic or niche
            let ragContext = '';
            try {
                const searchNiche = lead.spider_memory?.applied_tactic || lead.spider_verdict?.pain || lead.name;
                ...
                if (results && results.length > 0) {
                    const texts = results.map(r => r.payload?.text_chunk).filter(Boolean);
                    if (texts.length > 0) {
                        ragContext = `\n[CONOCIMIENTO PROPIO RELEVANTE (RAG)]:\nAplica estos principios extraídos de nuestra base de datos para este nicho:\n${texts.join('\n---\n')}`;
                    }
                }
            } catch (ragError) {
                console.warn(`[MarioService] RAG Query non-blocking fail: ${ragError.message}`);
            }

            // 3. RLHF Injection
            let rlhfBlock = '';
            if (previousFailedStrategies && previousFailedStrategies.length > 0) {
                const feedbackLogs = previousFailedStrategies.map((fs, idx) => 
            ...
            const systemPrompt = `Eres un sistema autónomo de inteligencia artificial operando como el "War Room" (Técnico + Estratega + Closer) de la agencia ${settings?.agencyName || 'Mariosweb'}.
Tu objetivo principal es cerrar ventas B2B asimétricas, actuando en representación de ${settings?.senderName || 'Gino'}.

[IDENTIDAD DE LA AGENCIA Y OFERTA]:
Contexto Global: ${agencyContext}
...

{
  "internal_reasoning": {
    ...
    "rag_sources_cited": "Principio del documento de conocimiento (Qdrant) aplicado, o 'N/A'.",
...
3. El JSON debe ser válido e interpretable directamente. No agregues backticks de markdown (como \\\`\\\`\\\`json) por fuera del objeto.`
            ...
            console.log(`[MarioService] Ejecutando War Room para Lead: ${leadId}. RLHF Activo: ${previousFailedStrategies.length > 0}`);

            // 5. OpenAI Call (Strict JSON)
            const engineInfo = AIService.getEngine();
            ...
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    ...
                    { role: "user", content: `Analiza al lead ${lead.name} y formula el armamento JSON para el ataque.` }
                ]
            });
            ...
            try {
                ...
                console.error(`[MarioService] Failed to parse JSON from OpenAI:`, rawJsonResponse);
