...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response) {
      ...
      if (status >= 500) {
        AlertService.error(
          "Fallo de conexión con el servidor. Intente de nuevo más tarde.",
          message,
        );
      ...
      AlertService.error(
        "Error de Red: No se puede conectar con Leads Pro AI. Verifique su conexión y el estado del servidor.",
      );
    } else {
      // Algo sucedió configurando la petición
      AlertService.error(
        "Error del sistema al procesar la petición.",
        error.message,
      );
    }
...
export const getLeadsBySearch = (searchId) =>
  api.get(`/history/${searchId}/leads`);
export const getGlobalStats = () => api.get("/stats");
export const updateLeadStatus = (id, status, note) =>
  api.patch(`/leads/${id}/status`, { status, note });
...
export const askAi = (
  ...
  campaignId = null,
  sessionId = null,
) => api.post("/ai/chat", { query, history, leadId, campaignId, sessionId });
export const getSpiderAnalysis = (leadId) =>
  api.get(`/ai/spider-analysis/${leadId}`);
...
export const triggerDeepVision = (leadId) =>
  api.post(`/vortex/deep-vision/${leadId}`);
export const getEnrichmentStatus = (leadId) =>
  api.get(`/vortex/status/${leadId}`);
export const resetVortex = (leadId) => api.post(`/vortex/reset/${leadId}`);

export const exportUrl = (searchId, format) =>
  `http://localhost:5000/api/export/${searchId}/${format}`;
...
export const saveAgencyContext = (data) =>
  api.post("/settings/agency-context", data);

export const getAgencySettings = () => api.get("/settings/agency");
